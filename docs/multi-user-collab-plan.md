...
- **B4 — Report-segment panel** (`<LetterSegmentCard>`): convert `variant`, `summary`, `content`, `delivery_day_override_id` to `useInstantField` + `patchReportSegment`. Two callsites in the workspace (slot 3 when segment opened from group, slot 5 when opened from an action).
- **B5 — Postgres_changes reducer + remote-delete toast:** subscribe `letters-workspace` channel to row updates on `inspection_letters`, `letter_groups`, `actions`, `report_segments`. Pipe through reducers that update local state by id+column. On DELETE of currently-selected record, close panel + toast attribution from peers. Address the typed-then-quick-switch edge case noted in B2/B3.
...
- **Typed-then-quick-switch edit loss.** If a user types into letter A then within 400 ms selects letter B, the closure capturing letter A's id may be replaced by the time `onCommitRef.current` fires, leading to the pending edit being saved to letter B instead. Currently mitigated only for `LetterFieldsCard` via `key={letterState.id}` remount (pending debounce is cancelled — edit lost, not misattributed). Group panel and action patcher don't have this guard yet. Proper fix: add flush-on-unmount to `useInstantField` (cleanup runs `commitNow` with the OLD closure).
...
- **Realtime publication on `inspection_action_ending_assignments`.** Not currently in the publication; `patchActionEndingAssignments` writes succeed but won't fan out to other clients via postgres_changes until added in B5 (or a follow-up migration).
