...
create table public.ending_variables (
...
create trigger ending_variables_set_updated_at before update on public.ending_variables
  for each row execute function public.set_updated_at();

create table public.ending_variable_values (
  id uuid primary key default uuid_generate_v4(),
...
create trigger ending_variable_values_set_updated_at before update on public.ending_variable_values
  for each row execute function public.set_updated_at();
create index ending_variable_values_variable_idx on public.ending_variable_values(variable_id);
...
create table public.ending_frameworks (
  id uuid primary key default uuid_generate_v4(),
  name text not null unique,
  sort_order int not null default 0,
  created_at timestamptz not null default now(),
...
create trigger ending_frameworks_set_updated_at before update on public.ending_frameworks
  for each row execute function public.set_updated_at();

create table public.ending_framework_blocks (
  id uuid primary key default uuid_generate_v4(),
...
create trigger ending_framework_blocks_set_updated_at before update on public.ending_framework_blocks
  for each row execute function public.set_updated_at();
create index ending_framework_blocks_framework_idx on public.ending_framework_blocks(framework_id);
create index ending_framework_blocks_parent_idx on public.ending_framework_blocks(parent_block_id);

-- At most one condition per (block, variable) since AND-ing two values for
-- the same variable can never match.
create table public.ending_block_conditions (
  id uuid primary key default uuid_generate_v4(),
...
create trigger ending_block_conditions_set_updated_at before update on public.ending_block_conditions
  for each row execute function public.set_updated_at();
create index ending_block_conditions_block_idx on public.ending_block_conditions(block_id);

create table public.ending_logic_rules (
  id uuid primary key default uuid_generate_v4(),
  framework_id uuid not null references public.ending_frameworks(id) on delete cascade,
  sort_order int not null default 0,
  created_at timestamptz not null default now(),
...
create trigger ending_logic_rules_set_updated_at before update on public.ending_logic_rules
  for each row execute function public.set_updated_at();
create index ending_logic_rules_framework_idx on public.ending_logic_rules(framework_id);

create table public.ending_logic_rule_conditions (
  id uuid primary key default uuid_generate_v4(),
...
create trigger ending_logic_rule_conditions_set_updated_at before update on public.ending_logic_rule_conditions
  for each row execute function public.set_updated_at();
create index ending_logic_rule_conditions_rule_idx on public.ending_logic_rule_conditions(rule_id);

create table public.inspection_action_ending_assignments (
  id uuid primary key default uuid_generate_v4(),
...
create trigger inspection_action_ending_assignments_set_updated_at before update on public.inspection_action_ending_assignments
  for each row execute function public.set_updated_at();
create index inspection_action_ending_assignments_action_idx on public.inspection_action_ending_assignments(action_id);
