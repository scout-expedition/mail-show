-- Smart Variables — constraints, indexes, and the smart_ref variable
-- shape. The new 'smart_variable' enum value lives in the preceding
-- migration (20260520120000_endings_smart_variables.sql); the split
-- exists because Postgres rejects (SQLSTATE 55P04) referencing a freshly
-- added enum value in the same transaction it was added in, and supabase
-- applies each migration file as its own transaction.

-- 1) Relax the name_shape constraint so smart_variable docs carry a name.
--
...
-- 2) Relax the per-kind singleton index so users can create many
...
-- 3) Smart variable names are unique across smart variables (so the
...
-- 4) Extend ending_variables.kind to allow 'smart_ref' and add the
